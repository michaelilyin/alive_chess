...
namespace AliveChessLibrary.Net
{
    public class NetworkDataStream
    {
        ...
        private void RecognizeCommand(BytePackage bp)
        {
            ...
            if (IsValueInRange(commandName, StatisticDownLimit, StatisticUpLimit))
                bp.CommandType = CommandType.StatisticCommand.ToString();
            if (IsValueInRange(commandName, ErrorDownLimit, ErrorUpLimit))
                bp.CommandType = CommandType.ErrorCommand.ToString();
        }

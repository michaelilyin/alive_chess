...
namespace AliveChessClient.NetLayer.Main
{
    ...
    public class QueryManager
    {
        ...
        //Шаг игрока
        public static void SendMoveUnit(Battle battle, byte[] move, bool ok)
        {
            MoveUnitRequest message = new MoveUnitRequest();
            message.Move = move;
            message.BattleID = battle.Id;
            message.OpponentID = battle.Respondent.Id;
            message.Ok = ok;
            ClientApplication.Instance.Transport.Send<MoveUnitRequest>(message);
        }

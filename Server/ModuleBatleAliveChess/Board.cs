...
namespace ModuleBatleAliveChess
{
    ...
    public class BoardModFighters
    {
        ...
        private GlobalC Gl = new GlobalC();
        ...
        {
            get { return myZobristKeys[myMemoryIndex]; }
        }
        ...
        public void ValidateMove(byte from, byte to, byte promote) // ПРОВЕРКА ДВИЖЕНИЯ
        {
            ...
            const string CASTLING_NA_MOVED = "Castling not available, either king or rook has been moved before";
                // рокировка не доступна
            const string CASTLING_NA_THREAT = "Castling not available, threat on a square in the path";
                //на пути рокировки что то есть
            ...
            const string ILLEGAL_CAPTURE = "Opponent piece on destination square";
                // Противник находится на клетке куда возможен ход
            ...
            if ((BoardC.CollisionMask[from, to] & ~myEmpty) != 0)
                throw new Exception(PIECE_COLLISION);
                    //Check obstacles, can be done before knowing face since Knight paths are not in the collision mask.
            ...
            if (face == Gl.King)
            {
                if (ThreatOnSquareAfterMove(to, from, to))
                    throw new Exception(MOVE_INTO_CHECK); //угроза на квадрате после движения

                if (BoardC.CollisionMask[from, to] != 0)
                    //If Castling (King moves two squares, collmask will have a bit set) король перемещается на 2 клетки и мустанавливается маска
                {
                    byte rookpos = (byte) (myPlayer*56);
                    if (to%8 > 4) rookpos += 7;

                    if ((myCastlings[myMemoryIndex] & BoardC.Bit[rookpos]) == 0)
                        throw new Exception(CASTLING_NA_MOVED); //рокировка не доступна

                    if ((BoardC.CollisionMask[from, rookpos] & ~myEmpty) == 0)
                        //If free path если длинна свободного места
                    {
        ...
        private bool ValidatePseudoMove(byte from, byte to, byte face) //проверка псевдо движения!!
        {
            ...
            if (face == Gl.King)
            {
                ...
                if (BoardC.CollisionMask[from, to] != 0)
                    //If Castling (King moves two squares, collmask will have a bit set)
                {
                    byte rookpos = (byte) (myPlayer*56);
                    if (to%8 > 4) rookpos += 7;
                    ...
                    if ((BoardC.CollisionMask[from, rookpos] & ~myEmpty) == 0)
                        //If free path если длинна свободного пути
                    {
        ...
        private bool ThreatOnSquareAfterMove(byte square, byte from, byte to) //угроза на клетку после хода
        {
            for (byte src = 0; src < 64; src++) //Go through all (myOpponent) pieces пройти по всей доске
            {
                ...
                if (src == to)
                    continue; //The piece is captured if the move is done клетка захвачена если движение выполнено
            ...
            if (square < from)
            {
                if ((from - square)%9 == 0)
                {
                    ...
                    for (; x <= 7 && y <= 7; x++, y++)
                    {
                        ...
                        {
                            if (myBoard[myOpponent, dst] == Gl.getBishop(0) ||
                                myBoard[myOpponent, dst] == Gl.getQueen(0))
                                return true;
                            else
                                break;
                        }
            ...
            for (byte from = 0; from < 64; from++)
            {
                ...
                if (face == Gl.getRook(0) || face == Gl.getQueen(0))
                {
                    ...
                    for (; x <= 7; x++)
                    {
                        ...
                        if ((threatmask & BoardC.Bit[to]) != 0 &&
                            !ThreatOnSquareAfterMove(myKingPos[myPlayer], from, to))
                        {
                ...
                if (face == Gl.BPawn)
                {
                    ...
                    if (x >= 1)
                    {
                        ...
                        if (myBoard[myOpponent, to] != Gl.Empty || to == myPassants[myMemoryIndex])
                            if (((threatmask & BoardC.Bit[to]) != 0 || to == myPassants[myMemoryIndex]) &&
                                !ThreatOnSquareAfterMove(myKingPos[myPlayer], from, to))
                            {
                                moves[count].from = from;
                                moves[count].to = to;
                                ...
                                {
                                    moves[count - 1].promote = Gl.getQueen(0);
                                    moves[count - 1].cmp = BoardC.FaceValue[myBoard[myOpponent, to]] - 10 +
                                                           BoardC.FaceValue[Gl.getQueen(0)] - BoardC.FaceValue[Gl.BPawn];

                                    moves[count] = moves[count - 1];
                                    moves[count].promote = Gl.getKnight(0);
                                    moves[count].cmp = BoardC.FaceValue[myBoard[myOpponent, to]] - 10 +
                                                       BoardC.FaceValue[Gl.getKnight(0)] - BoardC.FaceValue[Gl.BPawn];
                                    count++;

                                    moves[count] = moves[count - 1];
                                    moves[count].promote = Gl.getRook(0);
                                    moves[count].cmp = BoardC.FaceValue[myBoard[myOpponent, to]] - 10 +
                                                       BoardC.FaceValue[Gl.getRook(0)] - BoardC.FaceValue[Gl.BPawn];
                                    count++;

                                    moves[count] = moves[count - 1];
                                    moves[count].promote = Gl.getBishop(0);
                                    moves[count].cmp = BoardC.FaceValue[myBoard[myOpponent, to]] - 10 +
                                                       BoardC.FaceValue[Gl.getBishop(0)] - BoardC.FaceValue[Gl.BPawn];
                                    count++;
                                }
                            }
                    }
                        ...
                        if (myBoard[myOpponent, to] != Gl.Empty || to == myPassants[myMemoryIndex])
                                ...
                                !ThreatOnSquareAfterMove(myKingPos[myPlayer], from, to))
                            {
                                ...
                                if (to >= 56) //Promotion
                                {
                                    moves[count - 1].promote = Gl.getQueen(0);
                                    moves[count - 1].cmp = BoardC.FaceValue[myBoard[myOpponent, to]] - 10 +
                                                           BoardC.FaceValue[Gl.getQueen(0)] - BoardC.FaceValue[Gl.WPawn];

                                    moves[count] = moves[count - 1];
                                    moves[count].promote = Gl.getKnight(0);
                                    moves[count].cmp = BoardC.FaceValue[myBoard[myOpponent, to]] - 10 +
                                                       BoardC.FaceValue[Gl.getKnight(0)] - BoardC.FaceValue[Gl.WPawn];
                                    count++;

                                    moves[count] = moves[count - 1];
                                    moves[count].promote = Gl.getRook(0);
                                    moves[count].cmp = BoardC.FaceValue[myBoard[myOpponent, to]] - 10 +
                                                       BoardC.FaceValue[Gl.getRook(0)] - BoardC.FaceValue[Gl.WPawn];
                                    count++;

                                    moves[count] = moves[count - 1];
                                    moves[count].promote = Gl.getBishop(0);
                                    moves[count].cmp = BoardC.FaceValue[myBoard[myOpponent, to]] - 10 +
                                                       BoardC.FaceValue[Gl.getBishop(0)] - BoardC.FaceValue[Gl.WPawn];
                                    count++;
                                }
                            }
                    }
        ...
        public void DoMove(Move move) //, out byte capture, out byte capture_pos, out byte passant, out ulong oddmoves)
        {
            ...
            if (myBoard[myOpponent, move.to] != Gl.Empty || (face >= Gl.WPawn && move.to == myPassants[myMemoryIndex]))
                //Capture?
            {
                ...
                if (myBoard[myOpponent, move.to] != Gl.Empty) //Ordinary capture обыкновение захвата
                {
                    ...
                    if (myCaptures[myMemoryIndex] == Gl.getRook(0))
                        myCastlings[myMemoryIndex] &= ~BoardC.Bit[move.to];
                            //Clear myCastling if a rook is captured эта лодья теперь не сможнт зделать рокировку
                }
                else //En passant
                {
                    myCapturePositions[myMemoryIndex] = (byte) ((move.from/8)*8 + move.to%8);
                        //Same row as capturing pawn and same column as to-square координаты от куда начнётся ход
                    if (myPlayer == Gl.White)
                        myCaptures[myMemoryIndex] = Gl.BPawn;
                    else
                        myCaptures[myMemoryIndex] = Gl.WPawn;
                    myEmpty |= BoardC.Bit[myCapturePositions[myMemoryIndex]];
                        //This pos will be left empty, so remove it from the board клетка занята
                }
                ...
                myZobristKeys[myMemoryIndex] ^=
                    ZobristC.PositionKeys[myOpponent, myCaptures[myMemoryIndex], myCapturePositions[myMemoryIndex]];
            ...
            if (face == Gl.getRook(0))
                myCastlings[myMemoryIndex] &= ~BoardC.Bit[move.from]; //Clear the castlemask if a rook moves

            if (myPassants[myMemoryIndex] != 64)
                myZobristKeys[myMemoryIndex] ^= ZobristC.PassantKeys[myPassants[myMemoryIndex]];
        ...
        {
            myMaterial *= -1;
                //Change material balance to reflect myPlayer balanceИзмените материальный баланс, чтобы отразить баланс myPlayer
            myPlayer = myOpponent;
            myOpponent = (byte) (1 - myPlayer);
        ...
        public void SetBoardFEN(string fen)
        {
            ...
            if (arr.Length >= 10)
            {
                ...
                if (arr[9].IndexOf(Gl.PieceFace(Gl.White, Gl.getQueen(0))) != -1)
                    myCastlings[myMemoryIndex] |= BoardC.Bit[0];
                if (arr[9].IndexOf(Gl.PieceFace(Gl.Black, Gl.King)) != -1) myCastlings[myMemoryIndex] |= BoardC.Bit[63];
                if (arr[9].IndexOf(Gl.PieceFace(Gl.Black, Gl.getQueen(0))) != -1)
                    myCastlings[myMemoryIndex] |= BoardC.Bit[56];
            }

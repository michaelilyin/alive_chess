...
IF @@TRANCOUNT > 0
ALTER TABLE [dbo].[chess_empire] DROP CONSTRAINT [DF_chess_empire_LastEditDate]
GO
IF @@ERROR <> 0 
...
IF @@TRANCOUNT > 0
ALTER TABLE [dbo].[chess_empire] DROP COLUMN [LastEditDate]
GO
IF @@ERROR <> 0 
     ROLLBACK TRANSACTION;
...
IF @@TRANCOUNT > 0
ALTER TABLE [dbo].[chess_empire] DROP CONSTRAINT [DF_chess_empire_CreationDate]
GO
IF @@ERROR <> 0 
     ROLLBACK TRANSACTION;
...
IF @@TRANCOUNT > 0
ALTER TABLE [dbo].[chess_empire] DROP COLUMN [CreationDate]
GO
IF @@ERROR <> 0 
     ROLLBACK TRANSACTION;
...
IF @@TRANCOUNT > 0
IF EXISTS (SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID(N'[dbo].[chess_empire_Tombstone]') and TYPE = N'U') 
   DROP TABLE [dbo].[chess_empire_Tombstone];

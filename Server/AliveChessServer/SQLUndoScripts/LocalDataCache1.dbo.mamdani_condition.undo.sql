...
IF @@TRANCOUNT > 0
ALTER TABLE [dbo].[mamdani_condition] DROP CONSTRAINT [DF_mamdani_condition_LastEditDate]
GO
IF @@ERROR <> 0 
...
IF @@TRANCOUNT > 0
ALTER TABLE [dbo].[mamdani_condition] DROP COLUMN [LastEditDate]
GO
IF @@ERROR <> 0 
     ROLLBACK TRANSACTION;
...
IF @@TRANCOUNT > 0
ALTER TABLE [dbo].[mamdani_condition] DROP CONSTRAINT [DF_mamdani_condition_CreationDate]
GO
IF @@ERROR <> 0 
     ROLLBACK TRANSACTION;
...
IF @@TRANCOUNT > 0
ALTER TABLE [dbo].[mamdani_condition] DROP COLUMN [CreationDate]
GO
IF @@ERROR <> 0 
     ROLLBACK TRANSACTION;
...
IF @@TRANCOUNT > 0
IF EXISTS (SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID(N'[dbo].[mamdani_condition_Tombstone]') and TYPE = N'U') 
   DROP TABLE [dbo].[mamdani_condition_Tombstone];

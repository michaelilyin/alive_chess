...
IF @@TRANCOUNT > 0
ALTER TABLE [dbo].[resource_vault] DROP CONSTRAINT [DF_resource_vault_LastEditDate]
GO
IF @@ERROR <> 0 
...
IF @@TRANCOUNT > 0
ALTER TABLE [dbo].[resource_vault] DROP COLUMN [LastEditDate]
GO
IF @@ERROR <> 0 
     ROLLBACK TRANSACTION;
...
IF @@TRANCOUNT > 0
ALTER TABLE [dbo].[resource_vault] DROP CONSTRAINT [DF_resource_vault_CreationDate]
GO
IF @@ERROR <> 0 
     ROLLBACK TRANSACTION;
...
IF @@TRANCOUNT > 0
ALTER TABLE [dbo].[resource_vault] DROP COLUMN [CreationDate]
GO
IF @@ERROR <> 0 
     ROLLBACK TRANSACTION;
...
IF @@TRANCOUNT > 0
IF EXISTS (SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID(N'[dbo].[resource_vault_Tombstone]') and TYPE = N'U') 
   DROP TABLE [dbo].[resource_vault_Tombstone];

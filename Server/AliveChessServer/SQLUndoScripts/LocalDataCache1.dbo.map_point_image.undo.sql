...
IF @@TRANCOUNT > 0
ALTER TABLE [dbo].[map_point_image] DROP CONSTRAINT [DF_map_point_image_LastEditDate]
GO
IF @@ERROR <> 0 
...
IF @@TRANCOUNT > 0
ALTER TABLE [dbo].[map_point_image] DROP COLUMN [LastEditDate]
GO
IF @@ERROR <> 0 
     ROLLBACK TRANSACTION;
...
IF @@TRANCOUNT > 0
ALTER TABLE [dbo].[map_point_image] DROP CONSTRAINT [DF_map_point_image_CreationDate]
GO
IF @@ERROR <> 0 
     ROLLBACK TRANSACTION;
...
IF @@TRANCOUNT > 0
ALTER TABLE [dbo].[map_point_image] DROP COLUMN [CreationDate]
GO
IF @@ERROR <> 0 
     ROLLBACK TRANSACTION;
...
IF @@TRANCOUNT > 0
IF EXISTS (SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID(N'[dbo].[map_point_image_Tombstone]') and TYPE = N'U') 
   DROP TABLE [dbo].[map_point_image_Tombstone];

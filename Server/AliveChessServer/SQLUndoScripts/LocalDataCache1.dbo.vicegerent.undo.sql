...
IF @@TRANCOUNT > 0
ALTER TABLE [dbo].[vicegerent] DROP CONSTRAINT [DF_vicegerent_LastEditDate]
GO
IF @@ERROR <> 0 
...
IF @@TRANCOUNT > 0
ALTER TABLE [dbo].[vicegerent] DROP COLUMN [LastEditDate]
GO
IF @@ERROR <> 0 
     ROLLBACK TRANSACTION;
...
IF @@TRANCOUNT > 0
ALTER TABLE [dbo].[vicegerent] DROP CONSTRAINT [DF_vicegerent_CreationDate]
GO
IF @@ERROR <> 0 
     ROLLBACK TRANSACTION;
...
IF @@TRANCOUNT > 0
ALTER TABLE [dbo].[vicegerent] DROP COLUMN [CreationDate]
GO
IF @@ERROR <> 0 
     ROLLBACK TRANSACTION;
...
IF @@TRANCOUNT > 0
IF EXISTS (SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID(N'[dbo].[vicegerent_Tombstone]') and TYPE = N'U') 
   DROP TABLE [dbo].[vicegerent_Tombstone];

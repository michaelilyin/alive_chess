/****
Этот сценарий SQL был создан с помощью диалогового окна "Настройка 
синхронизации данных". Сценарий содержит операторы, которые 
создают столбцы отслеживания изменений, таблицу удаленных 
элементов и триггеры базы данных сервера. Эти объекты базы данных 
необходимы службам синхронизации для успешной синхронизации 
данных между базами данных на клиенте и на сервере. Дополнительные 
сведения см. в справке "Настройка сервера базы данных для синхронизации".
****/


IF @@TRANCOUNT > 0
set ANSI_NULLS ON 
set QUOTED_IDENTIFIER ON 

GO
BEGIN TRANSACTION;


IF @@TRANCOUNT > 0
ALTER TABLE [dbo].[base_point] 
ADD [LastEditDate] DateTime NULL CONSTRAINT [DF_base_point_LastEditDate] DEFAULT (GETUTCDATE()) WITH VALUES
GO
IF @@ERROR <> 0 
     ROLLBACK TRANSACTION;


IF @@TRANCOUNT > 0
ALTER TABLE [dbo].[base_point] 
ADD [CreationDate] DateTime NULL CONSTRAINT [DF_base_point_CreationDate] DEFAULT (GETUTCDATE()) WITH VALUES
GO
IF @@ERROR <> 0 
     ROLLBACK TRANSACTION;


IF @@TRANCOUNT > 0
IF NOT EXISTS (SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID(N'[dbo].[base_point_Tombstone]')) 
BEGIN 
CREATE TABLE [dbo].[base_point_Tombstone]( 
    [base_point_id] Int NOT NULL,
    [DeletionDate] DateTime NULL
) ON [PRIMARY] 
END 

GO
IF @@ERROR <> 0 
     ROLLBACK TRANSACTION;


IF @@TRANCOUNT > 0
ALTER TABLE [dbo].[base_point_Tombstone] ADD CONSTRAINT [PKDEL_base_point_Tombstone_base_point_id]
   PRIMARY KEY CLUSTERED
    ([base_point_id])
    ON [PRIMARY]
GO
IF @@ERROR <> 0 
     ROLLBACK TRANSACTION;


IF @@TRANCOUNT > 0
IF EXISTS (SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID(N'[dbo].[base_point_DeletionTrigger]') AND type = 'TR') 
   DROP TRIGGER [dbo].[base_point_DeletionTrigger] 

GO
CREATE TRIGGER [dbo].[base_point_DeletionTrigger] 
    ON [dbo].[base_point] 
    AFTER DELETE 
AS 
SET NOCOUNT ON 
UPDATE [dbo].[base_point_Tombstone] 
    SET [DeletionDate] = GETUTCDATE() 
    FROM deleted 
    WHERE deleted.[base_point_id] = [dbo].[base_point_Tombstone].[base_point_id] 
IF @@ROWCOUNT = 0 
BEGIN 
    INSERT INTO [dbo].[base_point_Tombstone] 
    ([base_point_id], DeletionDate)
    SELECT [base_point_id], GETUTCDATE()
    FROM deleted 
END 

GO
IF @@ERROR <> 0 
     ROLLBACK TRANSACTION;


IF @@TRANCOUNT > 0
IF EXISTS (SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID(N'[dbo].[base_point_UpdateTrigger]') AND type = 'TR') 
   DROP TRIGGER [dbo].[base_point_UpdateTrigger] 

GO
CREATE TRIGGER [dbo].[base_point_UpdateTrigger] 
    ON [dbo].[base_point] 
    AFTER UPDATE 
AS 
BEGIN 
    SET NOCOUNT ON 
    UPDATE [dbo].[base_point] 
    SET [LastEditDate] = GETUTCDATE() 
    FROM inserted 
    WHERE inserted.[base_point_id] = [dbo].[base_point].[base_point_id] 
END;
GO
IF @@ERROR <> 0 
     ROLLBACK TRANSACTION;


IF @@TRANCOUNT > 0
IF EXISTS (SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID(N'[dbo].[base_point_InsertTrigger]') AND type = 'TR') 
   DROP TRIGGER [dbo].[base_point_InsertTrigger] 

GO
CREATE TRIGGER [dbo].[base_point_InsertTrigger] 
    ON [dbo].[base_point] 
    AFTER INSERT 
AS 
BEGIN 
    SET NOCOUNT ON 
    UPDATE [dbo].[base_point] 
    SET [CreationDate] = GETUTCDATE() 
    FROM inserted 
    WHERE inserted.[base_point_id] = [dbo].[base_point].[base_point_id] 
END;
GO
IF @@ERROR <> 0 
     ROLLBACK TRANSACTION;
COMMIT TRANSACTION;
